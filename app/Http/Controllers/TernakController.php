...
class TernakController extends Controller
{
    ...
    /* ------------------------------------------------------------------ */
    /*  PENGAJUAN VERIFIKASI – PER DATA & SEMUA SEKALIGUS                  */
    /* ------------------------------------------------------------------ */

    /**
     * Ajukan satu data ternak untuk verifikasi (status 0/3 → 1).
     */
    ...
    /**
     * Ajukan SEMUA data ternak (draft/revisi) milik kecamatan ini sekaligus.
     */
    ...
    /**
     * Batalkan pengajuan satu data ternak (status 1 → 0).
     */
    public function batalAjukan(string $id)
    {
        ...
        if ($valid) {
            Ternak::where('id', $id)->update(['status_pengajuan' => 0]);
        }

        return redirect('ternak')->with('success', 'Pengajuan data berhasil dibatalkan.');
    }

    /**
     * Batalkan SEMUA pengajuan yang masih pending milik kecamatan ini.
     */
    public function batalSemua()
    {
        ...
        DB::table('ternaks')
            ...
            ->where('ternaks.tahun', $now)
            ->where('ternaks.status_pengajuan', 1)
            ->update(['ternaks.status_pengajuan' => 0]);

        return redirect('ternak')->with('success', 'Semua pengajuan berhasil dibatalkan.');
    }
